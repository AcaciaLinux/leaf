...
Package::Package(){
    FUN();
    LOGF("[Package][Package] Created new unknown package");
}
...
Package::Package(const std::string& name, const std::string& version){
    ...
    LOGF("[Package][Package] Created package \"" + getFullName() + "\"");
}

Package* Package::CreateFromLocal(std::string path){
    FUN();
    LOGF("[Package][Package] Creating new local package from path \"" + path + "\"");
...
void Package::addProvidedFile(std::string filepath){
    ...
    {//Strip the path to remove leading "/"
        ...
        LOGF("[Package][Package] " + msg + filepath + "\"");
    }
